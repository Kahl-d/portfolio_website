...
export default function Home() {
  ...
  const aboutTop = useTransform(scrollYProgress, [0, 0.5], ["12.5vh", "0vh"]);
  const aboutLeft = useTransform(scrollYProgress, [0, 0.5], ["10vw", "0vw"]);
  const aboutRight = useTransform(scrollYProgress, [0, 0.5], ["10vw", "0vw"]);
  const aboutBottom = useTransform(scrollYProgress, [0, 0.5], ["50vh", "0vh"]);
  const aboutBorderRadius = useTransform(scrollYProgress, [0, 0.5], ["1rem", "0rem"]);

  // Background opacity - card color fades to background color
  const aboutBgOpacity = useTransform(scrollYProgress, [0.2, 0.5], [0, 1]);

  // Text animations
  const aboutInitialTextScale = useTransform(scrollYProgress, [0, 0.5], [1, 1.2]);
  const aboutExtraContentOpacity = useTransform(scrollYProgress, [0.4, 0.7], [0, 1]);
  const aboutExtraContentY = useTransform(scrollYProgress, [0.4, 0.7], [30, 0]);

  // ===== OTHER BOXES FADE OUT =====
  const otherBoxesOpacity = useTransform(scrollYProgress, [0, 0.25], [1, 0]);
  const otherBoxesScale = useTransform(scrollYProgress, [0, 0.25], [1, 0.95]);
  const gridContainerOpacity = useTransform(scrollYProgress, [0, 0.3], [1, 0]);
